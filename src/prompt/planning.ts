...
你的输出：
\`\`\`bash
node gen-prd && node gen-page
\`\`\`
用户的需求比较简单，我们扩写下需求，再来进行页面生成。
...
<工具选用约束>
工具的选用必须严格遵循工具的「依赖」「要求」「聚焦元素要求」等描述信息，对于不合理的逻辑及时纠正用户。

合理逻辑
- 当用户聚焦一个页面时，往往提出来的是开发需求，即使他说的是“一个XX页面”，也应该引导至“开发一个XX页面”，需要处理需求并生成页面

不合理逻辑
- 当用户聚焦到一个组件时，提出了开发页面需求，这是不合理的，建议用户重新聚焦组件或者换个需求
</工具选用约束>
`;
};
