...
<examples>
  ...
  <example>
    ...
    <assistant>
      ...
      \`\`\`
      完整性检查：需要获取历史对话记录，了解之前的方案是什么，之前的方案是来自方案规划工具，不再重复调用方案规划工具，再调用execute-plan工具可以执行具体的方案。
    </assistant>
    <reason>未读取历史记录，无法知道"之前的方案"具体内容</reason>
  </example>
  ...
  <example>
    ...
    <assistant>
      ...
      \`\`\`
      完整性检查：需要获取历史对话记录，了解这个文档的内容，再调用optimize-doc工具优化文档内容。
    </assistant>
  </example>
</examples>
</当规划模式时>
...
<错误的返回示例>
  ...
  <example>
    ...
    <reason>
      用户使用了"这个"指代词，必须先读取历史记录了解具体是什么文档，否则无法进行有效优化。
    </reason>
  </example>
</错误的返回示例>
