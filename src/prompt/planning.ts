  ...
  <如何思考>
    ...
    3. 规划工具，
      3.1 如果当前需求是「修改组件/区块」
        ...
        3.1.2 判断是否需要新增组件，如果需要新增，则还需要「组件选型」来获取新增组件的配置文档
      ...
      3.4 如果当前需求仅为「开发、搭建」，那么对于组件/区块或页面的判断取决于当前的聚焦元素；
      3.5 如果当前需求对页面、组件的不满、不好、不够xx等类似表达时，应理解该需求为是3.4中的「开发、搭建」，并直接针对性调整；
  </如何思考>
  
  <关注>
    1. 如果需求中明确表达「搭建、开发、添加、新增、删除、修改」等意义相同的动词或类似表达时，必须调用相应工具来执行；
  </关注>
</针对当前领域如何规划工具>
...
\`\`\`
用户的需求比较简单，但是查询天气需要先知道用户当前的位置信息，所以需要先调用search-geo来获取地理位置信息，再调用get-weather来获取天气信息。
